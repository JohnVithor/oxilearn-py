...
/usr/bin/time -f "%e,%S,%U,%P,%M,%Z,%F,%R,%c,%w,%I,%O,%C" taskset --cpu-list 0 python scripts/run_rust_$target.py $seed false $verbose
# /usr/bin/time -f "%e,%S,%U,%P,%M,%Z,%F,%R,%c,%w,%I,%O,%C" taskset --cpu-list 0 python scripts/run_python_$target.py $seed false $verbose
# /usr/bin/time -f "%e,%S,%U,%P,%M,%Z,%F,%R,%c,%w,%I,%O,%C" taskset --cpu-list 0 python scripts/run_jax_$target.py $seed false $verbose