...
def create_objective(env_name, seed, verbose):
    ...
    def objective(trial) -> float:
        ...
        eval_env = make_vec_env(env_name, seed=seed+1, n_envs=1, vec_env_cls=DummyVecEnv)
        ...
        model = DQN(
            policy="MlpPolicy", env=vec_env, learning_rate=learning_rate, batch_size=batch_size,
            buffer_size=buffer_size, learning_starts=learning_starts, gamma=gamma,
            target_update_interval=target_update_interval, train_freq=train_freq,
            gradient_steps=gradient_steps, exploration_initial_eps=1.00,
            exploration_fraction=exploration_fraction,
            exploration_final_eps=exploration_final_eps,
            policy_kwargs={'net_arch': [256, 256]}, seed=seed+2
        )
