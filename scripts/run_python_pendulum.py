...
def main(seed, save, verbose):
    ...
    model = PPO(policy="MlpPolicy", env=vec_env, learning_rate=1e-3, gamma=0.9,
                n_steps=1024, n_epochs=10, gae_lambda=0.95, ent_coef=0.0, 
                clip_range=0.2, use_sde=True, sde_sample_freq=4, seed=seed)
    ...
    if save:
        ...
        save_file(model.policy.mlp_extractor.policy_net.state_dict(), './safetensors-python/policy_weights.safetensors')
        save_file(model.policy.mlp_extractor.value_net.state_dict(), './safetensors-python/value_weights.safetensors')
        save_file(model.policy.state_dict(), './safetensors-python/p.safetensors')

    return model.num_timesteps, evaluate_policy(model, eval_env, n_eval_episodes=eval_size, render=True)
